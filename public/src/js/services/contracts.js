...
angular.module('insight.contracts')
	...
	.factory('Contracts',
	function(FantasyGoldCoreLib, Opcodes, Networks, Constants) {

		var CONTRACT_CALL = 194;
		var CONTRACT_CREATE = 193;

		return {
			isValidFantasyGoldAddress: function (address) {
                return FantasyGoldCoreLib.Address.isValid(address, Constants.NETWORK)
			},
			getBitAddressFromContractAddress: function (contractAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        ...
                        checksum = FantasyGoldCoreLib.crypto.Hash.sha256sha256(new FantasyGoldCoreLib.deps.Buffer(networkId + contractAddress, 'hex')),
                        hexBitAddress = networkId + contractAddress + checksum.toString('hex').slice(0, 8);

                    return FantasyGoldCoreLib.encoding.Base58.encode(new FantasyGoldCoreLib.deps.Buffer(hexBitAddress, 'hex'));
            ...
            getEthAddressFromBitAddress: function (bitAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        networkId = network.pubkeyhash.toString(16),
                        hexBitAddress = FantasyGoldCoreLib.encoding.Base58.decode(bitAddress).toString('hex');
			...
			getContractOpcodesString: function (hex) {
				...
				var contractCode = new FantasyGoldCoreLib.deps.Buffer(hex, 'hex'),
					ops = [];
			...
			getContractBytecode: function (hex) {
				...
				try {
					...
					var script = FantasyGoldCoreLib.Script(hex);
			...
			getContractAddress: function (txId, num) {
				var reverseTxId = txId.match(/.{2}/g).reverse().join(""),
					buf = new FantasyGoldCoreLib.deps.Buffer(4);

				buf.writeUInt32LE(num, 0);

				var nHex = buf.toString('hex'),
					addr = reverseTxId + nHex,
					bufferAddress = FantasyGoldCoreLib.crypto.Hash.sha256ripemd160(new FantasyGoldCoreLib.deps.Buffer(addr, 'hex'));
	...
	.factory('ERC20ContractInfo', function($resource, $window) {
		...
		return $resource($window.apiPrefix + '/fgc20/:contractAddress',
            {
                address: '@address'
            },
		{
