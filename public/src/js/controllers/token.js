...
function($routeParams, $rootScope, $location, ERC20ContractInfo, ERC20Transfers, ERC20AddressBalances, ERC20Holders, ContractsRepository, SolidityCoder, Web3Utils, Contracts, BigNumber) {
	...
	if (!Web3Utils.isAddress($routeParams.address) && !Contracts.isValidFantasyGoldAddress($routeParams.address)) {
    ...
    if ($routeParams.a) {
        ...
        if (Contracts.isValidFantasyGoldAddress($routeParams.a)) {
    ...
    var getEthConvertedAddress = function (address) {
    	...
    	try {
    		...
    		if (Contracts.isValidFantasyGoldAddress(address)) {
