...
angular.module("insight", ["ngResource", "ngRoute", "ngLodash", "ui.bootstrap", "ui.route", "monospaced.qrcode", "gettext", "angularMoment", "insight.system", "insight.socket", "insight.blocks", "insight.chart", "insight.charts", "insight.richList", "insight.transactions", "insight.address", "insight.search", "insight.statistics", "insight.status", "insight.markets", "insight.connection", "insight.currency", "insight.messages", "insight.fantasygoldcorelib", "insight.bignumber", "insight.solidity_coder", "insight.web3_utils", "insight.contracts", "insight.opcodes", "insight.networks", "insight.token"]), angular.module("insight.system", ["ngScrollbars", "chart.js"]), angular.module("insight.socket", []), angular.module("insight.blocks", []), angular.module("insight.charts", []), angular.module("insight.chart", []), angular.module("insight.richList", []), angular.module("insight.transactions", ["ngScrollbars", "jQueryScrollbar"]), angular.module("insight.address", []), angular.module("insight.search", []), angular.module("insight.token", ["ngNumeraljs"]), angular.module("insight.statistics", ["ngNumeraljs"]), angular.module("insight.status", []), angular.module("insight.markets", []), angular.module("insight.connection", []), angular.module("insight.currency", []), angular.module("insight.messages", []), angular.module("insight.messages", []), angular.module("insight.fantasygoldcorelib", []), angular.module("insight.contracts", []), angular.module("insight.opcodes", []), angular.module("insight.networks", []), angular.module("insight.solidity_coder", []), angular.module("insight.web3_utils", []), angular.module("insight.bignumber", []), angular.module("insight.address").controller("AddressController", function ($scope, $rootScope, $document, $routeParams, $location, $window, Address, getSocket, Constants, ContractsInfo, Contracts, ERC20AddressBalances, ERC20ContractInfo, Web3Utils) {
  ...
  var _startSocket = function () {
      socket.on("fantasygoldd/addresstxid", function (data) {
        if (data.address === addrStr) {
          var base = $document.find("base"),
            beep = new Audio(base[0].href + "/sound/transaction.mp3");
          $rootScope.$broadcast("tx", data.txid), beep.play()
        }
      }), socket.emit("subscribe", "fantasygoldd/addresstxid", [addrStr])
    },
    _parseStorageRowType = function (hex, type) {
      switch (type) {
        case "string":
          var str = "",
            i = 0,
            l = hex.length;
          for ("0x" === hex.substring(0, 2) && (i = 2); l > i; i += 2) {
    ...
    _defineDefaultState = function (string, number) {
      var stringMatchUnread = string.match(/[^a-zA-Z0-9;:'".,\/\]\[?!&%#@)(_`><\s]/g) || [],
        stringMatchRead = string.match(/[a-zA-Z0-9;:'".,\/\]\[?!&%#@)(_`><]/g),
        isLastSymbolUnread = string[string.length - 1] === stringMatchUnread[0];
      return ~number.toString().indexOf("e") ? !isLastSymbolUnread && stringMatchUnread.length || !stringMatchRead ? self.STORAGE_CONST.DATA : self.STORAGE_CONST.STRING : self.STORAGE_CONST.NUMBER
    },
    _formStorageInfo = function () {
      var rows = [];
          ...
          for (var key in self.info.storage[row]) {
            ...
            if (self.info.storage[row].hasOwnProperty(key)) {
              for (var CONST in self.STORAGE_CONST) {
                ...
                newRow.values[constName] = _parseStorageRowType(self.info.storage[row][key], constName), newRow.keys[constName] = _parseStorageRowType(key, constName)
              }
              newRow.values.state = _defineDefaultState(newRow.values.string, newRow.values.number, newRow.values.address), newRow.keys.state = _defineDefaultState(newRow.keys.string, newRow.keys.number, newRow.keys.address), rows.push(newRow)
            }
          }
      return rows
    },
  ...
  socket.on("connect", function () {
    ...
    _stopSocket()
  }), self.params = $routeParams, self.findOne = function () {
    $rootScope.currentAddr = addrStr, _startSocket(), ERC20AddressBalances.query({
      balanceAddress: addrStr
    }, function (balances) {
      balances && balances.length && (self.balances = balances.map(function (balance) {
        return balance.contract && (balance.contract.contract_address_base = Contracts.getBitAddressFromContractAddress(balance.contract.contract_address)), balance
      }))
    }), Address.get({
      addrStr: addrStr
    }, function (address) {
      $rootScope.titleDetail = address.addrStr.substring(0, 7) + "...", $rootScope.flashMessage = null, self.address = address;
      var ethAddress = Contracts.getEthAddressFromBitAddress(addrStr);
      ethAddress && (ERC20ContractInfo.get({
        contractAddress: ethAddress
      }, function (data) {
        data && (self.erc20ContractInfo = data)
      }), ContractsInfo.get({
        contractAddressStr: ethAddress
      }, function (info) {
        info && (self.info = info, self.opcodesStr = Contracts.getContractOpcodesString(info.code), self.storage.rows = _formStorageInfo(), self.storage.storageLength = Object.keys(info.storage).length, self.storage.viewRows = Constants.STORAGE_ROWS)
      }, function (e) {
        console.log("e", e)
      }))
    }, function (e) {
      400 === e.status ? $rootScope.flashMessage = "Invalid Address: " + addrStr : 503 === e.status ? $rootScope.flashMessage = "Backend Error. " + e.data : $rootScope.flashMessage = "Address Not Found", $location.path("/")
    })
  ...
  }, self.tokenDropdownOpen = !1, self.toggleDropdownTokenTracker = function () {
    self.tokenDropdownOpen = !self.tokenDropdownOpen
  }
}), angular.module("insight.blocks").controller("BlocksController", function ($scope, $rootScope, $routeParams, $location, moment, Block, Blocks, BlockByHeight) {
  var self = this;
  self.loading = !1, self.date = null, self.datepicker = {
    date: null,
    format: "yyyy-MM-dd",
    isOpened: !1,
    dateOptions: {
  ...
  }, $routeParams.blockHeight && BlockByHeight.get({
    ...
    newValue !== oldValue && scope.BC.datepicker.isOpened && (self.datepicker.date = newValue.getTime(), $location.path("/blocks-date/" + moment(newValue).format("YYYY-MM-DD")))
  }), self.openDatepicker = function (e) {
    e.preventDefault(), e.stopPropagation(), self.datepicker.isOpened = !0
  }, self.disableDatepicker = function (data) {
    var date = data.date,
      mode = data.mode;
    return "day" === mode && (0 === date.getDay() || 6 === date.getDay())
  }, self.loadList = function () {
    if (self.loading = !0, $routeParams.blockDate && (self.detail = "On " + $routeParams.blockDate), $routeParams.startTimestamp) {
      var d = new Date(1e3 * $routeParams.startTimestamp),
        m = d.getMinutes();
      10 > m && (m = "0" + m), self.before = " before " + d.getHours() + ":" + m
    }
    $rootScope.titleDetail = self.detail, Blocks.get({
      blockDate: $routeParams.blockDate,
      startTimestamp: $routeParams.startTimestamp
    }, function (res) {
      self.loading = !1;
      var date = new Date(res.pagination.current);
      self.date = new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes(), date.getUTCSeconds()), self.datepicker.date = self.date.getTime(), self.blocks = res.blocks, self.pagination = res.pagination
    })
  }, self.findOne = function () {
    self.loading = !0, Block.get({
      blockHash: $routeParams.blockHash
    }, function (block) {
      $rootScope.titleDetail = block.height, $rootScope.flashMessage = null, self.loading = !1, self.block = block
    }, function (e) {
    ...
    var present = new Date,
      presentDate = moment.utc([present.getUTCFullYear(), present.getUTCMonth(), present.getUTCDate()]),
      currentDate = moment.utc(self.pagination.current);
...
}), angular.module("insight.contracts").controller("ContractsController", function ($scope, $rootScope, $routeParams, $location, Contracts) {
  try {
    var addrStr = Contracts.getBitAddressFromContractAddress($routeParams.contractAddressStr);
    return $location.path("/address/" + addrStr), !1
  } catch (e) {
    return $rootScope.flashMessage = "Invalid Address: " + $routeParams.contractAddressStr, $location.path("/"), !1
  }
}), angular.module("insight.currency").controller("CurrencyController", function ($scope, $rootScope, Currency, Constants, BigNumber, $filter) {
  var self = this;
  $rootScope.currency = {
  ...
  $rootScope.currency.getConvertion = function (value) {
    ...
    $rootScope.currency.symbol = Constants.CURRENCY[currency], localStorage.setItem("insight-currency", Constants.CURRENCY[currency]), "USD" === currency ? Currency.get({}, function (res) {
      $rootScope.currency.factor = $rootScope.currency.bitstamp = res.data.bitstamp
    }) : "mBTC" === currency ? $rootScope.currency.factor = 1e3 : "bits" === currency ? $rootScope.currency.factor = 1e6 : $rootScope.currency.factor = 1
...
}), angular.module("insight.system").controller("HeaderController", function ($scope, $rootScope, $route, gettextCatalog, amMoment, getSocket, Block, $templateCache, Constants, $location) {
  function _getMenu() {
    return [{
  ...
  var self = this,
    socket = getSocket($scope);
  self.defaultLanguage = Constants.DEFAULT_LANGUAGE, self.menu = _getMenu(), self.isActiveMenuItem = function (item) {
    return -1 !== $location.path().search(item) ? "active" : ""
  ...
  socket.on("connect", function () {
    ...
    gettextCatalog.currentLanguage = self.defaultLanguage = isoCode, amMoment.changeLocale(isoCode), localStorage.setItem("insight-language", isoCode), $templateCache.remove(currentPageTemplate), self.menu = _getMenu(), $route.reload()
  }, $rootScope.isCollapsed = !0
}), angular.module("insight.system").controller("IndexController", function ($scope, $rootScope, $window, $timeout, moment, getSocket, Blocks, TransactionsByDays, Constants, Status, $q, StatisticsTotalSupply, MarketsInfo) {
  var self = this,
    socket = getSocket($scope);
  self.blockchainInfo = null, self.marketsInfo = {
    price_usd: 0,
    price_btc: 0,
    market_cap_usd: 0
  }, self.txs = [], self.blocks = [], self.chartDays = Constants.CHART_DAYS, self.rewardCurrency = Constants.CURRENCY.FGC, self.scrollConfig = {
    autoHideScrollbar: !1,
    axis: "y",
    theme: "custom",
    advanced: {
    ...
    callbacks: {
      onBeforeUpdate: function () {
        ...
        heightList > maxHeight ? this.style.height = maxHeight + "px" : this.style.height = heightList + "px"
      }
    }
  }, self.chartOptions = {
    series: ["Transactions"],
  ...
  var _getBlocks = function () {
      Blocks.get({
        ...
        self.blocks = res.blocks, self.blocksLength = res.length
      })
    },
    socketRequestTimer = null,
    _startSocket = function () {
      socket.emit("subscribe", "inv"), socket.on("tx", function (tx) {
        tx.createTime = Date.now(), self.txs.unshift(tx), self.txs.length > Constants.TRANSACTION_DISPLAYED && (self.txs.length = Constants.TRANSACTION_DISPLAYED)
      }), socket.on("info", function (data) {
        self.blockchainInfo = data.info, self.blockchainInfo.supply = data.supply, self.blockchainInfo.stakingInfo = data.stakingInfo
      }), socket.on("block", function () {
        clearTimeout(socketRequestTimer), socketRequestTimer = setTimeout(function () {
          _getBlocks()
        }, 2e3 * Math.random())
      }), socket.on("markets_info", function (marketsInfo) {
        self.marketsInfo = marketsInfo
  ...
  socket.on("connect", function () {
    ...
    TransactionsByDays.query({
      ...
      for (; response.length < self.chartDays;) response.push({
        date: moment().subtract(self.chartDays - (self.chartDays - response.length), "days").format("YYYY-MM-DD"),
        transaction_count: 0
      });
      self.lastTransactionsList = response.reverse(), self.chartOptions.labels = self.lastTransactionsList.map(function (item) {
  ...
  var _getInfo = function () {
      $q.all([Status.get({
        q: "getInfo"
      }).$promise, StatisticsTotalSupply.get({
        format: "object"
      }).$promise, Status.get({
        q: "getStakingInfo"
      }).$promise]).then(function (results) {
        results[0] && results[1] && results[2] && (self.blockchainInfo = results[0].info, self.blockchainInfo.stakingInfo = results[2], self.blockchainInfo.supply = results[1].supply)
      })
    },
    _getMarketsInfo = function () {
      return MarketsInfo.get({}, function (response) {
        response && (self.marketsInfo = response)
      })
    };
  self.index = function () {
    _getInfo(), _getMarketsInfo(), _getBlocks(), _startSocket()
  }
  ...
  qrcode.callback = function (data) {
    _scanStop();
    var str = 0 === data.indexOf("fantasygold:") ? data.substring(8) : data;
...
}), angular.module("insight.search").controller("SearchController", function ($location, $timeout, Block, Transaction, Address, BlockByHeight, ERC20ContractInfo, Contracts) {
  var self = this;
  self.loading = !1;
  var _badQuery = function () {
      self.badQuery = !0, $timeout(function () {
        self.badQuery = !1
      }, 2e3)
    },
    _resetSearch = function () {
      self.q = "", self.loading = !1
    };
  self.search = function () {
    var q = self.q.trim();
    self.badQuery = !1, self.loading = !0, Block.get({
      blockHash: q
    }, function () {
      ...
      Transaction.get({
        ...
        Address.get({
          ...
          _resetSearch(), $location.path("address/" + bitAddress)
        }, function () {
          isFinite(q) ? BlockByHeight.get({
            ...
            self.loading = !1, _badQuery()
          }) : ERC20ContractInfo.get({
            contractAddress: q
          }, function () {
            _resetSearch(), $location.path("/token/" + q)
          }, function () {
            self.loading = !1, _badQuery()
          })
        })
...
}), angular.module("insight.transactions").controller("SendRawTransactionController", function ($scope, $http, $filter, SendRawTransaction, lodash) {
  var timeoutId = null;
  $scope.status = "ready", $scope.error = null, $scope.isEmpty = !1, $scope.rawTransaction, $scope.txid = "", $scope.jqueryScrollbarOptions = {}, $scope.send = function () {
    if (clearState(), validateRawTransaction()) {
      var postData = {
        rawtx: $scope.rawTransaction.trim()
      };
      SendRawTransaction.send(postData, function (successfullResponse) {
        return "string" != typeof successfullResponse.txid ? ($scope.status = "error", void($scope.error = ": the transaction was sent but no transaction id was got back")) : ($scope.status = "sent", $scope.txid = successfullResponse.txid, void clearDataAfterSuccessfullSend())
    ...
    return "error" === $scope.status ? !0 : !1
  }, $scope.checkIsEmpty = function () {
    return $scope.isEmpty
  }, $scope.isStatusSent = function () {
    return "sent" === $scope.status ? !0 : !1
  }, $scope.$on("$destroy", function () {
    clearState()
  });
  var clearState = function () {
      $scope.status = "ready", $scope.isEmpty = !1, $scope.error = null, $scope.txid = "", clearTimeout(timeoutId)
    },
    validateRawTransaction = function () {
      var rawTrx = $scope.rawTransaction;
      return rawTrx ? lodash.isString(rawTrx) && /^[0-9a-fA-F]+$/.test(rawTrx) ? !0 : ($scope.status = "error", $scope.error = ": the transaction hex is not valid", !1) : ($scope.isEmpty = !0, !1)
    },
    clearDataAfterSuccessfullSend = function () {
      $scope.rawTransaction = null, timeoutId = setTimeout(function () {
        clearState(), $scope.$apply()
      }, 12e4)
    }
}), angular.module("insight.statistics").controller("StatisticsController", function ($scope, $routeParams, Statistics, StatisticsByDaysTransactions, StatisticsByDaysOutputs, StatisticsByDaysFees, StatisticsByDaysDifficulty, StatisticsByDaysStakes, Statistics24Hours, gettextCatalog, $filter, Constants, StatisticChart, MarketsInfo) {
  var self = this,
    factories = {
      transactions: {
  ...
  }, self.chartDays = $routeParams.days, self.chartType = $routeParams.type, self.marketCurrency = Constants.CURRENCY.USD, self.marketPrice = 0;
  var statisticChart = new StatisticChart(self.chartDays);
  self.chartOptions = statisticChart.chartOptions, self.daysButtons = statisticChart.daysButtons, $scope.$on("chart-create", function (evt, chart) {
    ...
    Statistics24Hours.get(function (response) {
      ...
      response && response.price_usd && (self.marketPrice = response.price_usd)
    })
  }
}), angular.module("insight.status").controller("StatusController", function ($scope, Status, Sync, getSocket) {
  var self = this,
    socket = getSocket($scope);
  self.getStatus = function (q) {
    Status.get({
      ...
      self.loaded = 1, angular.extend(self, d)
    }, function (e) {
      self.error = "API ERROR: " + e.data
    })
  };
  var _onSyncUpdate = function (sync) {
      sync.startTs || (sync.startTs = Date.now()), self.sync = sync
    },
    _startSocket = function () {
      socket.emit("subscribe", "sync"), socket.on("status", function (sync) {
        _onSyncUpdate(sync)
...
}), angular.module("insight.token").controller("TokenController", function ($routeParams, $rootScope, $location, ERC20ContractInfo, ERC20Transfers, ERC20AddressBalances, ERC20Holders, ContractsRepository, SolidityCoder, Web3Utils, Contracts, BigNumber) {
  if (!Web3Utils.isAddress($routeParams.address) && !Contracts.isValidFantasyGoldAddress($routeParams.address)) return $rootScope.flashMessage = "Invalid Address: " + $routeParams.address, $location.path("/e404").replace(), !1;
  var contractEthAddress = $routeParams.address;
  if (!Web3Utils.isAddress(contractEthAddress)) return contractEthAddress = Contracts.getEthAddressFromBitAddress($routeParams.address), $location.path("/token/" + contractEthAddress).replace(), !1;
  var self = (Contracts.getBitAddressFromContractAddress($routeParams.address), this),
    BALANCE_OF_METHOD_HASH = "70a08231",
    ALLOWANCE_METHOD_HASH = "dd62ed3e";
  self.tokenInfo = {}, self.transfers = {}, self.holders = {}, self.filterByAddress = null, $routeParams.a && (Web3Utils.isAddress($routeParams.a) && (self.filterByAddress = Contracts.getBitAddressFromContractAddress($routeParams.a)), Contracts.isValidFantasyGoldAddress($routeParams.a) && (self.filterByAddress = $routeParams.a)), self.addressBalance = null, self.readSmartContractTab = {
    balanceOf: {
      owner_address: "",
      inProcess: !1,
      owner_error: "",
  ...
  }, self.contractAddress = $routeParams.address, self.tab = $routeParams.tab && -1 !== ["transfers", "holders", "read-smart-contract"].indexOf($routeParams.tab) ? $routeParams.tab : "transfers";
  ...
  var _getTransfers = function (offset) {
      ERC20Transfers.get({
        ...
        self.transfers = trList, self.transfers.pages = self.transfers.count && self.transfers.limit ? Math.ceil(self.transfers.count / self.transfers.limit) : 0;
      })
    },
    _getHolders = function (offset) {
      return ERC20Holders.get({
        address: contractEthAddress,
        offset: offset
      }).$promise.then(function (holderList) {
        self.holders = holderList, self.holders.pages = self.holders.count && self.holders.limit ? Math.ceil(self.holders.count / self.holders.limit) : 0
      })
    },
    _loadTokenInfo = function () {
      ERC20ContractInfo.get({
      ...
      }), self.filterByAddress && ERC20AddressBalances.get({
        contractAddress: contractEthAddress,
  ...
  self.init = function () {
    _loadTokenInfo(), _loadTabContent()
  }, self.paginate = function (offset) {
    self[self.tab].limit && self[self.tab].pages > offset / self[self.tab].limit && offset >= 0 && self[self.tab].offset !== offset && _loadTabContent(offset)
  }, self.setTab = function (tabName) {
    self.tab !== tabName && (self.tab = tabName, $location.path("/token/" + $routeParams.address + "/" + tabName, !1), _loadTabContent())
  };
  var getEthConvertedAddress = function (address) {
    try {
      ...
      if (address = address.toLowerCase(), Web3Utils.isAddress(address)) return address
    } catch (e) {
      console.log(e)
    }
    return null
  ...
  self.getAddressBalance = function () {
    ...
    if (balanceOfData.inProcess) return !1;
    var userAddress = balanceOfData.owner_address,
      processAddress = getEthConvertedAddress(userAddress);
    return processAddress ? (balanceOfData.owner_error = "", balanceOfData.inProcess = !0, ContractsRepository.call.get({
      address: contractEthAddress,
      hash: BALANCE_OF_METHOD_HASH + SolidityCoder.encodeParam("address", Web3Utils.toAddress(processAddress))
    }).$promise.then(function (info) {
      if (balanceOfData.requested_address = userAddress, balanceOfData.process_address = processAddress, balanceOfData.owner_address = "", balanceOfData.inProcess = !1, info && info.executionResult) try {
    ...
    })) : (balanceOfData.owner_error = "Invalid address", !1)
  }, self.getAllowanceAmount = function () {
    var allowanceData = self.readSmartContractTab.allowance;
    if (allowanceData.inProcess) return !1;
    var userOwnerAddress = allowanceData.owner_address,
      processOwnerAddress = getEthConvertedAddress(userOwnerAddress);
    if (!processOwnerAddress) return allowanceData.owner_error = "Invalid address", !1;
    allowanceData.owner_error = "";
    var userSpenderAddress = allowanceData.spender_address,
      processSpenderAddress = getEthConvertedAddress(userSpenderAddress);
    return processSpenderAddress ? (allowanceData.spender_error = "", allowanceData.inProcess = !1, ContractsRepository.call.get({
      address: contractEthAddress,
      ...
      if (allowanceData.owner_requested_address = allowanceData.owner_address, allowanceData.owner_process_address = processOwnerAddress, allowanceData.spender_requested_address = allowanceData.spender_address, allowanceData.spender_process_address = processSpenderAddress, allowanceData.owner_address = "", allowanceData.spender_address = "", allowanceData.inProcess = !1, info.executionResult) try {
        var decodedBalance = SolidityCoder.decodeParam("uint256", info.executionResult.output);
        allowanceData.balance = decodedBalance.toString(10)
      } catch (e) {
        allowanceData.balance = 0
      }
    })) : (allowanceData.spender_error = "Invalid address", !1)
  }, self.paginateData = {
    transfers: {
      paginate_has_error: !1,
      paginate_page: ""
    },
    holders: {
      paginate_has_error: !1,
    ...
    self.paginateData[self.tab].paginate_has_error = !1, self.paginateData[self.tab].paginate_page = ""
    ...
    return /^\d+$/.test(value) ? (value = parseInt(e.target.value, 10), void(value && !isNaN(value) && countPages >= value ? (self.paginate(self[self.tab].limit * (value - 1)), this.resetPaginate()) : self.setPaginateError(!0))) : self.setPaginateError(!0)
  }
}), angular.module("insight.system").controller("TokensSearchController", function ($scope, $rootScope, $route, gettextCatalog, ContractsRepository, lodash) {
  var self = this;
  self.init = function () {
    getContractsList()
  };
  var currentRequestUniqueId, getDefaultSearchResult = function () {
    return {
      count: 0,
      items: []
    }
  };
  self.query = "", self.error = "", self.inProcess = !1, self.searchResult = getDefaultSearchResult(), self.contractsList = [];
  var getContractsList = function () {
      ContractsRepository.contractsList.get({}, function (res) {
        if (res && res.count && res.items) {
          var transformedItems = [];
          res.items.forEach(function (item) {
            item.exception || !item.name && !item.symbol || transformedItems.push(item)
    ...
    debounced = lodash.debounce(function (requestId) {
      return currentRequestUniqueId !== requestId ? !1 : void ContractsRepository.search.get({
        query: lodash.trim(self.query)
      }, function (res) {
        self.inProcess = !1, currentRequestUniqueId === requestId && (self.searchResult = res)
      }, function () {
        self.error = "Request Error!"
      })
    }, 550);
  self.search = function () {
    lodash.trim(self.query) ? self.inProcess || self.changeQuery() : self.error = "Search field is required!"
  }, self.reset = function () {
    currentRequestUniqueId = lodash.uniqueId(), self.query = "", self.searchResult = {
      count: 0,
      items: []
    }
  }, self.changeQuery = function () {
    self.error = "", self.inProcess = !0, currentRequestUniqueId = lodash.uniqueId(), _.trim(self.query) ? debounced(currentRequestUniqueId) : (self.reset(), self.inProcess = !1)
  }
}), angular.module("insight.transactions").controller("TransactionsController", function ($scope, $rootScope, $routeParams, $location, Transaction, TransactionsByBlock, TransactionsByAddress, Contracts, $q, ERC20ContractInfo, Web3Utils) {
  var self = this,
    pageNum = 0,
    pagesTotal = 1,
    COIN = 1e8;
  self.loading = !1, self.loadedBy = null, self.isCopied = !1, self.scrollConfig = {
    autoHideScrollbar: !1,
    theme: "custom",
    advanced: {
      updateOnContentResize: !0
  ...
  var _aggregateItems = function (txId, items) {
      ...
      for (var l = items.length, ret = [], tmp = {}, u = 0, i = 0; l > i; i++) {
        var notAddr = !1;
        if (items[i].scriptSig && !items[i].addr && (items[i].addr = "Unparsed address [" + u++ + "]", items[i].notAddr = !0, notAddr = !0), items[i].scriptPubKey && !items[i].scriptPubKey.addresses) {
          var contractAddress;
          items[i].scriptPubKey.hex && (contractAddress = Contracts.getContractAddressByHex(txId, items[i].n, items[i].scriptPubKey.hex)), contractAddress && (items[i].contractAddress = contractAddress, items[i].contractAddressBit = Contracts.getBitAddressFromContractAddress(contractAddress)), items[i].scriptPubKey.addresses = ["Unparsed address [" + u++ + "]"], items[i].notAddr = !0, notAddr = !0
        }
        if (items[i].scriptPubKey && items[i].scriptPubKey.addresses.length > 1) items[i].addr = items[i].scriptPubKey.addresses.join(","), ret.push(items[i]);
        else {
          var addr = items[i].addr || items[i].scriptPubKey && items[i].scriptPubKey.addresses[0];
          tmp[addr] || (tmp[addr] = {}, tmp[addr].valueSat = 0, tmp[addr].count = 0, tmp[addr].addr = addr, tmp[addr].items = []), tmp[addr].isSpent = items[i].spentTxId, tmp[addr].doubleSpentTxID = tmp[addr].doubleSpentTxID || items[i].doubleSpentTxID, tmp[addr].doubleSpentIndex = tmp[addr].doubleSpentIndex || items[i].doubleSpentIndex, tmp[addr].dbError = tmp[addr].dbError || items[i].dbError, tmp[addr].valueSat += Math.round(items[i].value * COIN), tmp[addr].items.push(items[i]), tmp[addr].notAddr = notAddr, tmp[addr].contractAddress = items[i].contractAddress || null, tmp[addr].contractAddressBit = items[i].contractAddressBit || null, items[i].unconfirmedInput && (tmp[addr].unconfirmedInput = !0), tmp[addr].count++
        }
      }
    ...
    _getContractBytecode = function (tx) {
      for (var items = tx.vout, l = items.length, i = 0; l > i; i++)
    ...
    contractsInfoCache = {},
    addEvent = function (tx, logItem) {
      return $q(function (resolve) {
        contractsInfoCache[logItem.address] || (contractsInfoCache[logItem.address] = ERC20ContractInfo.get({
    ...
    asyncProcessERC20TX = function (tx) {
      var deferred = $q.defer(),
        tokenPromises = [];
      if (tx.tokenEvents = [], tx.isfgc20Transfer && tx.receipt && tx.receipt.length)
        for (var i = 0; i < tx.receipt.length; i++) {
          ...
          if (receiptItem && receiptItem.log && receiptItem.log.length)
            for (var j = 0; j < receiptItem.log.length; j++) {
              ...
              logItem && logItem.topics && 3 === logItem.topics.length && "ddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef" === logItem.topics[0] && tokenPromises.push(addEvent(tx, logItem))
            }
        }
      return $q.all(tokenPromises).then(function () {
        deferred.resolve(tx)
      }), deferred.promise
    },
    _processTX = function (tx) {
      var contractBytecodeInfo = _getContractBytecode(tx);
      tx.vinSimple = _aggregateItems(tx.txid, tx.vin), tx.voutSimple = _aggregateItems(tx.txid, tx.vout), self.loading = !1, contractBytecodeInfo && (tx.contractBytecode = contractBytecodeInfo.code, tx.contractBytecodeType = contractBytecodeInfo.type, tx.contractAsm = Contracts.getContractOpcodesString(tx.contractBytecode))
    },
    _paginate = function (data) {
      pagesTotal = data.pagesTotal, pageNum += 1;
      var promises = [];
      data.txs.forEach(function (tx) {
        promises.push(asyncProcessERC20TX(tx))
      }), $q.all(promises).then(function (results) {
        results.forEach(function (tx) {
          tx.showAdditInfo = !1, _processTX(tx), self.txs.push(tx)
        }), self.loading = !1
      })
    },
    _byBlock = function () {
    ...
    _byAddress = function () {
      ...
      Web3Utils.isAddress(address) && (address = Contracts.getBitAddressFromContractAddress(address)), TransactionsByAddress.get({
        address: address,
        pageNum: pageNum
      }, function (data) {
        _paginate(data)
    ...
    _findTx = function (txid) {
      Transaction.get({
        ...
        $rootScope.titleDetail = tx.txid.substring(0, 7) + "...", $rootScope.flashMessage = null, asyncProcessERC20TX(tx).then(function (tx) {
          self.tx = tx, _processTX(tx), self.txs.unshift(tx)
        })
      }, function (e) {
  ...
  self.findThis = function () {
    self.loading = !0, _findTx($routeParams.txId)
  }, self.load = function (from) {
    self.loadedBy = from, self.loadMore()
  }, self.loadMore = function () {
    if (pagesTotal > pageNum && !self.loading) switch (self.loading = !0, self.loadedBy) {
      case "address":
        _byAddress();
        break;
  ...
  }, (">" == $routeParams.v_type || "<" == $routeParams.v_type) && (self.from_vin = "<" == $routeParams.v_type ? !0 : !1, self.from_vout = ">" == $routeParams.v_type ? !0 : !1, self.v_index = parseInt($routeParams.v_index), self.itemsExpanded = !0), self.txs = [], $scope.$on("tx", function (event, txid) {
    _findTx(txid)
  })
}), angular.module("insight.address").factory("Address", function ($resource, $window) {
...
}), angular.module("insight.bignumber").factory("BigNumber", function ($window) {
  return $window.eth_libs.BigNumber
}), angular.module("insight.fantasygoldcorelib").factory("FantasyGoldCoreLib", function () {
  var FantasyGoldCoreLib = require("fantasygoldcore-lib");
  return FantasyGoldCoreLib
  ...
  return StatisticChart.prototype.changeChartColor = function (chart) {
    ...
    gradient.addColorStop(0, "rgba(46, 154, 208,0.5)"), gradient.addColorStop(1, "rgba(0, 0, 0,0.001)"), chart.chart.config.data.datasets[0].backgroundColor = gradient
...
}), angular.module("insight.contracts").factory("ContractsInfo", function ($resource) {
  return $resource(window.apiPrefix + "/contracts/:contractAddressStr/info")
}).factory("Contracts", function (FantasyGoldCoreLib, Opcodes, Networks, Constants) {
  ...
  return {
    ...
    getEthAddressFromBitAddress: function (bitAddress) {
      try {
        ...
        return hexBitAddress.slice(0, 2) !== networkId ? null : hexBitAddress.slice(2, -8)
      } catch (e) {
        return null
      }
    },
    getContractOpcodesString: function (hex) {
      for (var contractCode = new FantasyGoldCoreLib.deps.Buffer(hex, "hex"), ops = [], index = 0; index < contractCode.length; index++) {
        var currentOp = Opcodes.lookupOpcode(contractCode[index], !0);
        if (currentOp.pc = index, ops.push(currentOp), "PUSH" === currentOp.name.slice(0, 4)) {
          var pushDataLength = contractCode[index] - 95,
            pushData = contractCode.slice(index + 1, index + pushDataLength + 1);
          currentOp.pushData = pushData, index += pushDataLength
        }
      }
      for (var opcodesStr = "", i = 0; i < ops.length; i++) opcodesStr += ops[i].pushData ? " " + ops[i].name + (ops[i].pushData ? " 0x" + ops[i].pushData.toString("hex") : "") : " " + ops[i].name;
      return opcodesStr
    },
      ...
      try {
        ...
        if (script.chunks && script.chunks.length)
          for (var k = 0; k < script.chunks.length; k++)
            if (script.chunks[k] && script.chunks[k].opcodenum && -1 !== [CONTRACT_CALL, CONTRACT_CREATE].indexOf(script.chunks[k].opcodenum)) switch (script.chunks[k].opcodenum) {
              case CONTRACT_CALL:
                return {
                  code: script.chunks[k - 2].buf.toString("hex"), type: "Call"
                };
              case CONTRACT_CREATE:
                return {
                  code: script.chunks[k - 1].buf.toString("hex"), type: "Create"
                }
            }
      } catch (e) {}
      return null
...
}), angular.module("insight.system").factory("Global", [function () {}]).factory("Version", function ($resource) {
  return $resource(window.apiPrefix + "/version")
}), angular.module("insight.markets").factory("MarketsInfo", function ($resource) {
  return $resource(window.apiPrefix + "/markets/info", {
    q: "@q"
  })
}), angular.module("insight.networks").factory("Networks", function (Constants, FantasyGoldCoreLib) {
  return {
    getCurrentNetwork: function () {
      return FantasyGoldCoreLib.Networks.get(Constants.NETWORK)
    }
  }
}), angular.module("insight.opcodes").factory("Opcodes", function () {
  var codes = {
    0: ["STOP", 0, 0, 0, !1],
    1: ["ADD", 3, 2, 1, !1],
    2: ["MUL", 5, 2, 1, !1],
    ...
    50: ["ORIGIN", 2, 0, 1, !0],
    51: ["CALLER", 2, 0, 1, !0],
    52: ["CALLVALUE", 2, 0, 1, !0],
    53: ["CALLDATALOAD", 3, 1, 1, !0],
    54: ["CALLDATASIZE", 2, 0, 1, !0],
    55: ["CALLDATACOPY", 3, 3, 0, !0],
    56: ["CODESIZE", 2, 0, 1, !1],
    57: ["CODECOPY", 3, 3, 0, !1],
    58: ["GASPRICE", 2, 0, 1, !1],
    59: ["EXTCODESIZE", 20, 1, 1, !0],
    60: ["EXTCODECOPY", 20, 4, 0, !0],
    64: ["BLOCKHASH", 20, 1, 1, !0],
    65: ["COINBASE", 2, 0, 1, !0],
    66: ["TIMESTAMP", 2, 0, 1, !0],
    67: ["NUMBER", 2, 0, 1, !0],
    68: ["DIFFICULTY", 2, 0, 1, !0],
    69: ["GASLIMIT", 2, 0, 1, !0],
    80: ["POP", 2, 1, 0, !1],
    81: ["MLOAD", 3, 1, 1, !1],
    82: ["MSTORE", 3, 2, 0, !1],
    ...
    163: ["LOG", 375, 5, 0, !1],
    164: ["LOG", 375, 6, 0, !1],
    240: ["CREATE", 32e3, 3, 1, !0],
    241: ["CALL", 40, 7, 1, !0],
    242: ["CALLCODE", 40, 7, 1, !0],
    243: ["RETURN", 0, 2, 0, !1],
    244: ["DELEGATECALL", 40, 6, 1, !0],
    255: ["SUICIDE", 0, 1, 0, !1]
  };
  return {
    lookupOpcode: function (op, full) {
      var code = codes[op] ? codes[op] : ["INVALID", 0],
        opcode = code[0];
      return full && ("LOG" === opcode && (opcode += op - 160), "PUSH" === opcode && (opcode += op - 95), "DUP" === opcode && (opcode += op - 127), "SWAP" === opcode && (opcode += op - 143)), {
        name: opcode,
        fee: code[1],
        "in": code[2],
        out: code[3],
...
ScopedSocket.prototype.removeAllListeners = function (opts) {
...
}), angular.module("insight.solidity_coder").factory("SolidityCoder", function ($window) {
  return $window.eth_libs.solidity_coder
}), angular.module("insight.statistics").factory("Statistics", function ($resource, $window) {
...
}), angular.module("insight.transactions").factory("Transaction", function ($resource, $window) {
...
}).factory("TransactionsByBlock", function ($resource, $window) {
  return $resource($window.apiPrefix + "/txs", {
    block: "@block"
  })
}).factory("TransactionsByAddress", function ($resource, $window) {
  return $resource($window.apiPrefix + "/txs", {
    address: "@address"
  })
}).factory("Transactions", function ($resource, $window) {
  return $resource($window.apiPrefix + "/txs")
}).factory("TransactionsByDays", function ($resource, $window) {
  return $resource($window.apiPrefix + "/statistics/transactions", {
    days: "@days"
  })
}).factory("SendRawTransaction", function ($resource, $window) {
...
}), angular.module("insight.web3_utils").factory("Web3Utils", function ($window) {
  ...
  return {
    ...
    link: function (scope, element) {
      var clipboard = new $window.Clipboard(element[0]),
        ...
        copiedElement = angular.element('<div class="copied">' + translate + "</div>");
      element.before(copiedElement), clipboard.on("success", function (e) {
        scope.$apply(function () {
          copiedElement.addClass("active"), $timeout(function () {
            copiedElement.removeClass("active")
          }, 2e3), scope.ngclipboardSuccess({
            e: e
          })
        })
  ...
  return {
    ...
    link: function (scope, element, attrs) {
      var isTooltipDynamicVariable = void 0 !== scope.tooltipTitleVariable;
      if (scope.tooltipTitle || isTooltipDynamicVariable) {
        var tooltipOptions = scope.tooltipOptions || {},
          message = scope.tooltipTitle;
        isTooltipDynamicVariable && (message = scope.tooltipTitleVariable, scope.$watch("tooltipTitleVariable", function (value) {
          element.tooltipster("content", value), value ? element.tooltipster("enable") : element.tooltipster("disable");
        })), tooltipOptions.content = message, element.tooltipster(tooltipOptions), scope.$on("$destroy", function () {
...
}), angular.module("insight").constant("Constants", {
  CURRENCY: {
    FGC: "FGC",
    USD: "USD",
    mBTC: "mBTC",
  ...
  $location.path = function (path, reload) {
    if (reload === !1) var lastRoute = $route.current,
      un = $rootScope.$on("$locationChangeSuccess", function () {
        $route.current = lastRoute, un()
      });
    return original.apply($location, [path])
  }, gettextCatalog.currentLanguage = Constants.DEFAULT_LANGUAGE, amMoment.changeLocale(Constants.DEFAULT_LANGUAGE), $rootScope.$on("$routeChangeSuccess", function () {
    $rootScope.titleDetail = "", $rootScope.title = $route.current.title, $rootScope.isCollapsed = !0, $rootScope.currentAddr = null, $location.hash($routeParams.scrollTo), $anchorScroll()
  })
  ...
  gettextCatalog.setStrings("de_DE", {
    ...
    "Can't connect to fantasygoldd to get live updates from the p2p network. (Tried connecting to fantasygoldd at {{host}}:{{port}} and failed.)": "Es ist nicht möglich mit FantasyGoldd zu verbinden um live Aktualisierungen vom P2P Netzwerk zu erhalten. (Verbindungsversuch zu fantasygoldd an {{host}}:{{port}} ist fehlgeschlagen.)",
    Complete: "Vollständig",
    Confirmations: "Bestätigungen",
    "Contract Address:": "Bestätigungen:",
    ...
    'Insight is an <a href="https://insight.is/" target="_blank">open-source FantasyGold blockchain explorer</a> with complete REST and websocket APIs that can be used for writing web wallets and other apps that need more advanced blockchain queries than provided by fantasygoldd RPC. Check out the <a href="https://github.com/bitpay/insight-ui" target="_blank" class="ng-scope">source code</a>.': 'Insight ist ein <a href="https://insight.is/" target="_blank">Open Source FantasyGold Blockchain Explorer</a> tmit vollständigen REST und Websocket APIs um eigene Wallets oder Applikationen zu implementieren. Hierbei werden fortschrittlichere Abfragen der Blockchain ermöglicht, bei denen die RPC des FantasyGoldd nicht mehr ausreichen. Der aktuelle <a href="https://github.com/bitpay/insight-ui" target="_blank" class="ng-scope">Quellcode </a>tist auf Github zu finden.',
    ...
    "Total Supply:": "Insgesamt gesendet:",
    ...
    "Insight is an": "Insight ist ein",
    "open-source FantasyGold blockchain explorer": "Open Source FantasyGold Blockchain Explorer",
    "source code": " ist auf Github zu finden",
    ...
    Input: "Eingänge",
    "Last Block Hash (FantasyGold)": "Letzter Hash (FantasyGoldd)",
    ...
    "Can't connect to fantasygoldd to get live updates from the p2p network. (Tried connecting to fantasygoldd at {{host}}:{{port}} and failed.)": "No se pudo conectar a fantasygoldd para obtener actualizaciones en vivo de la red p2p. (Se intentó conectar a fantasygoldd de {{host}}:{{port}} y falló.)",
    Complete: "Completado",
    Confirmations: "Confirmaciones",
    "Contract Address:": "Confirmaciones:",
    Copied: "CopiedTest",
    ...
    Version: "Versión",
    "github issue tracker": "Github issue tracker",
    "Insight is an": "Insight  es un",
    "open-source FantasyGold blockchain explorer": "explorador de bloques de FantasyGold open-source",
    "source code": "el repositorio abierto de Github",
    ...
    Input: "Entrada",
    "Last Block Hash (FantasyGold)": "Último Bloque Hash (FantasyGoldd)",
    ...
    "Can't connect to fantasygoldd to get live updates from the p2p network. (Tried connecting to fantasygoldd at {{host}}:{{port}} and failed.)": "P2Pネットワークからライブ情報を取得するためにfantasygolddへ接続することができませんでした。({{host}}:{{port}} への接続を試みましたが、失敗しました。)",
    Coinbase: "コインベース",
    Complete: "完了",
    Confirmations: "検証数",
    "Contract Address:": "検証数:",
    ...
    "FantasyGold comes with a way of signing arbitrary messages.": "FantasyGoldには任意のメッセージを署名する昨日が備わっています。",
    ...
    "This form can be used to verify that a message comes from\r\n        a specific FantasyGold address.": "このフォームでは、メッセージが特定のFantasyGoldアドレスから来たかどうかを検証することができます。",
    "Time Offset": "時間オフセット",
    Timestamp: "タイムスタンプ",
    "Total Received": "総入金額",
    "Total Sent": "総送金額",
    ...
    Input: "入力",
    "Last Block Hash (FantasyGold)": "直前のブロックのハッシュ値 (FantasyGold)",
